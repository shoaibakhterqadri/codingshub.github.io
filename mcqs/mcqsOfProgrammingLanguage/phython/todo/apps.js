...
function addMCQs(){
  onAuthStateChanged (auth , async user => {
    if(user){
      try {
        const docRef = await addDoc(collection(db, "MCQsDataOfPhython"), {
...
onAuthStateChanged(auth , (user) => {
  if (user) {
    const q = query(collection(db, "MCQsDataOfPhython"), where("uid", "==", user.uid));
     onSnapshot(q, (querySnapshot) => {
      ...
      querySnapshot.forEach((doc) => {
       ...
       <h5 class="card-header"><pre>${doc.data().questionStore}</pre></h5>
       <div class="card-body">
       <h5 class="card-title"><pre>${doc.data().answerStore}</pre></h5>
         <p class="card-text"><pre>${doc.data().opt1Store}</pre></p>
         <p class="card-text"><pre>${doc.data().opt2Store}</pre></p>
         <p class="card-text"><pre>${doc.data().opt3Store}</pre></p>
         <p class="card-text"><pre>${doc.data().opt4Store}</pre></p>
...
async function deleteMcqs(id){
  const deleteRef= doc(db, "MCQsDataOfPhython", id );
