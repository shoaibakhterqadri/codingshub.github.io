...
function addMCQs(){
  onAuthStateChanged (auth , async user => {
    if(user){
      try {
        const docRef = await addDoc(collection(db, "MCQsDataOfReact"), {
...
onAuthStateChanged(auth , (user) => {
  if (user) {
    const q = query(collection(db, "MCQsDataOfReact"), where("uid", "==", user.uid));
...
async function deleteMcqs(id){
  const deleteRef= doc(db, "MCQsDataOfReact", id );
