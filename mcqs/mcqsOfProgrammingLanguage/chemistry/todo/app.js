...
async function addMCQs(){
      try {
        const docRef = await addDoc(collection(db, "MCQsDataOfChemistry"), {   
...
(async function(){
  try{
    ...
    const colRef = collection(db, "MCQsDataOfChemistry");
...
<button type="button" id="thirdOpt"  onclick="checkAnswerofMcqs(this,'${doc.data().opt3Store}', '${doc.data().answerStore}')"  class="list-group-item list-group-item-action">${doc.data().opt3Store}</pre></button>
<button type="button" id="fourthOpt"  onclick="checkAnswerofMcqs(this,'${doc.data().opt4Store}', '${doc.data().answerStore}')"  class="list-group-item list-group-item-action">${doc.data().opt4Store}</pre></button>
</div>
     </div>
   </div>  
     </span>
...
async function deleteMcqs(id){
  const deleteRef= doc(db, "MCQsDataOfChemistry", id );
