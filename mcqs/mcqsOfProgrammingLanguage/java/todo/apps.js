...
function addMCQs(){
  onAuthStateChanged (auth , async user => {
    if(user){
      try {
        const docRef = await addDoc(collection(db, "MCQsDataOfJava"), {
...
onAuthStateChanged(auth , (user) => {
  if (user) {
    const q = query(collection(db, "MCQsDataOfJava"), where("uid", "==", user.uid));
...
async function deleteMcqs(id){
  const deleteRef= doc(db, "MCQsDataOfJava", id );
